...
md1_CC = fitcdiscr(obs_train(:,QDA_CC_fet),grp_train,'DiscrimType','pseudoQuadratic');
[label,NegLoss, PBScore] = predict(md1_CC,obs_test(:,QDA_CC_fet));

accuracy_CC = (sum((grp_test-label) == 0)/length(grp_test))*100;

save(strcat('C:\Users\Nathan\Desktop\Thesis\My data\new data\QDA tests\test with both\accuracy_CC'), 'accuracy_CC');
