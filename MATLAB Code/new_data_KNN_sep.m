...
md1_CH1 = fitcknn(obs_train_ch1(:,KNN_fet_ch1),grp_train_ch1,'NumNeighbors',5);
[label_ch1,NegLoss, PBScore] = predict(md1_CH1,obs_test_ch1(:,KNN_fet_ch1));
accuracy_ch1 = (sum((grp_test_ch1-label_ch1) == 0)/length(grp_test_ch1))*100;

md1_CH2 = fitcknn(obs_train_ch2(:,KNN_fet_ch2),grp_train_ch2,'NumNeighbors',5);
[label_ch2,NegLoss, PBScore] = predict(md1_CH2,obs_test_ch2(:,KNN_fet_ch2));
accuracy_ch2 = (sum((grp_test_ch2-label_ch2) == 0)/length(grp_test_ch2))*100;

md1_CH3 = fitcknn(obs_train_ch3(:,KNN_fet_ch3),grp_train_ch3,'NumNeighbors',5);
[label_ch3,NegLoss, PBScore] = predict(md1_CH3,obs_test_ch3(:,KNN_fet_ch3));
accuracy_ch3 = (sum((grp_test_ch3-label_ch3) == 0)/length(grp_test_ch3))*100;

md1_CH4 = fitcknn(obs_train_ch4(:,KNN_fet_ch4),grp_train_ch4,'NumNeighbors',5);
[label_ch4,NegLoss, PBScore] = predict(md1_CH4,obs_test_ch4(:,KNN_fet_ch4));
accuracy_ch4 = (sum((grp_test_ch4-label_ch4) == 0)/length(grp_test_ch4))*100;

md1_CH5 = fitcknn(obs_train_ch5(:,KNN_fet_ch5),grp_train_ch5,'NumNeighbors',5);
[label_ch5,NegLoss, PBScore] = predict(md1_CH5,obs_test_ch5(:,KNN_fet_ch5));
...
save(strcat('C:\Users\Nathan\Desktop\Thesis\My data\new data\KNN tests\test with both\accuracy_sep'), 'accuracy_sep');
