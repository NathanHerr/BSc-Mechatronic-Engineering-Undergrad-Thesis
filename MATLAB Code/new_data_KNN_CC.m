...
md1_CC = fitcknn(obs_train(:,KNN_CC_fet),grp_train,'NumNeighbors',5);
[label,NegLoss, PBScore] = predict(md1_CC,obs_test(:,KNN_CC_fet));

accuracy_CC = (sum((grp_test-label) == 0)/length(grp_test))*100;

save(strcat('C:\Users\Nathan\Desktop\Thesis\My data\new data\KNN tests\test with both\accuracy_CC'), 'accuracy_CC');
