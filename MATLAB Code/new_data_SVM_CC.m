...
md1_CC = fitcecoc(obs_train(:,SVM_CC_fet),grp_train);
[label,NegLoss, PBScore] = predict(md1_CC,obs_test(:,SVM_CC_fet));

accuracy_CC = (sum((grp_test-label) == 0)/length(grp_test))*100;

save(strcat('C:\Users\Nathan\Desktop\Thesis\My data\new data\SVM tests\test with both\accuracy_CC'), 'accuracy_CC');
